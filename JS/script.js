...
const textItems = document.querySelectorAll(
  ".match-cards .card-items .match-card"
);
const cardsImage = document.querySelectorAll(
  ".cards .card-item .img-wrapper .img-container"
);
...
cardsImage.forEach((cardItem) => {
  ...
  cardItem.addEventListener("drop", (event) => {
    ...
    const text = document.querySelector(
      `.match-cards .card-items .match-card[data-index="${textId}"]`
    );
    if (index === textId) {
      const cardText = document.querySelector(
        `.cards .card-item .text[data-index="${textId}"]`
      );
      ...
      cardText.addEventListener("animationend", () => {
      ...
      audio.addEventListener("ended", () => {
        if (counter === cardsImage.length) {
          const text = document.querySelector(".text-card .score-text");
          text.textContent = `${counter}/${cardsImage.length}`;
          text.setAttribute("text", `${counter}/${cardsImage.length}`);
